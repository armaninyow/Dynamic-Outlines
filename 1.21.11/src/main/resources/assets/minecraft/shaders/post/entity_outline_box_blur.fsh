...
// NOTE: as of 1.21.6, Radius (and BlurDir) moved into a shared "BlurConfig" UBO block,
// declared identically here and in blur.vsh - UBOs are shared buffer objects bindable
// independently by any shader stage, so no changes to blur.vsh are needed for this
// fragment shader to read the same dynamically-set Radius value.
// NOTE: as of 1.21.9, core/screenquad.vsh no longer emits a per-vertex sampleStep varying;
// vanilla now computes it in-shader from the SamplerInfo UBO (InSize/OutSize), populated
// automatically by the post-chain pipeline. We compute sampleStep the same way vanilla's
// own updated box_blur.fsh does: (1/InSize) * BlurDir.
void main() {
